...
public class CacheLoaderTest {
    ...
    private final static Integer KEY = 1000; // 自定义key
    ...
    private void init() throws InterruptedException {
        loadingCache = CacheBuilder.newBuilder()
                .expireAfterWrite(2, TimeUnit.SECONDS) // 2秒内不写如数据
                .removalListener(new RemovalListener<Object, Object>() { // 事件回调
                    @Override
                    public void onRemoval(RemovalNotification<Object, Object> notification) {

                        LOGGER.info("删除原因={ }，删除 key={}",
                                notification.getCause().toString(),notification.getKey());
                    }
                })
